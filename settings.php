...
<body id='page'>
    ...
    </form>" ; //単語番号が指定されていて、設定が「削除」だった時 } elseif (!empty($_POST['word_id']) && 'delete'==$_POST['settings']) { echo "
    <form method=post action='https://word-note.main.jp/book.php'>
    ...
    </form>" ; //削除確認画面で「終わりだよもう何もかも」が来たら <?php elseif ($_POST['settings'] == 'owaridayomou-nanimo-kamo') : ?> include('./login_safe.php'); $delete=mysqli_query($link, "delete from words where word_id='" . $_POST['word_id'] . "' LIMIT 1" ); //設定ミスった時のためにLIMIT $count=mysqli_query($link, "select word_id from words where book_id='" . $_POST['book_id'] . "'" ); $count_words_all=0; while ($count_words=mysqli_fetch_assoc($count)) { //echo "残り単語数" .$count_words['word_id']; $count_words_all++; } //echo "全部で" .$count_words_all; echo "<p>
        【" . $_POST['question'] . "】を削除しました！
    </p>" ; <?php if ($count_words_all == 0) : ?> echo "
    ...
    where book_id='" . $_POST['book2'] . "'" ); <?php if (!$update_book) : ?> echo "updateできなかった" ; <?php else : ?> echo "【" . $before_book_name . "】を<br>【" . $_POST['newtitle'] . "】というタイトルに<br>変更しました！" ; echo "<form method=post action='https://word-note.main.jp/index.php'>
                ...
                <?php else : ?>
                     <?php if (!empty($result_bookid)) : ?> 
                        <?php echo $result_bookid; ?>e
                        cho " 不正な操作です <form method=post action='https://word-note.main.jp/book.php'>" ; echo "<input type=hidden name='openbook' value='" . $result_bookid . "'>" ; echo "<button id='return' class='clear_button'>
                    <img src='./img/iconmonstr-undo-1-32.png'>
                </button>
        </form>
    <?php else : ?>
        echo $_POST['book_name'] . "と" . $_POST['book_id'] . "と" . $_POST['settings']; echo "不正な操作かもしれないからTOPにいくよ
        <form method=post action='https://word-note.main.jp/index.php'>" ; echo "<button id='return' class='clear_button'>
                <img src='./img/iconmonstr-undo-1-32.png'>
            </button></form>
    <?php endif; ?>
<?php endif; ?>
